...
public final class Cost: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.costs].array {
        costs = items.map { Costs(json: $0) }
    }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.costs = aDecoder.decodeObject(forKey: SerializationKeys.costs) as? [Costs]
  }
